...
namespace EnyimRedux.Caching
{
    public partial class MemcachedClient
    {
        ...
        async Task IDistributedCache.SetAsync(string key, byte[] value, DistributedCacheEntryOptions options, CancellationToken token = default)
        {
            var expiration = GetExpiration(options);
        ...
        private string GetSlidingExpirationKey(string key)
        {
            return $"{key}-sliding-expiration";
        }
